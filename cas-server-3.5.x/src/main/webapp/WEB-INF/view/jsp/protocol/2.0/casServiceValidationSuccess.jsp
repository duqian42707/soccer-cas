        ...
        <c:if test="${fn:length(assertion.chainedAuthentications[fn:length(assertion.chainedAuthentications)-1].principal.attributes) > 0}">
            <cas:attributes>
                <c:forEach var="attr"
                    ...
                    <cas:${fn:escapeXml(attr.key)}><%=URLEncoder.encode(pageContext.getAttribute("val") == null
                            ? "Unknown" : pageContext.getAttribute("val").toString(), "UTF-8")%></cas:${fn:escapeXml(attr.key)}>
                </c:forEach>
            </cas:attributes>
        </c:if>
            <%-- 加入的自定义用户信息生成 - 结束 --%>
